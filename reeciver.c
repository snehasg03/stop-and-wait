...
int main(){
	...
	recieverAddr.sin_port = htons(atoi("4400"));
	recieverAddr.sin_addr.s_addr = inet_addr("127.0.0.1");
	...
	while(1){
		...
		if(err>0){
			...
			if(drop<2){
				...
				//drop ack
				counter;
				printf("Not Correctly recieved: %s, with seq number :%d\n",recvPacket.data,recvPacket.seqnum);
				continue;
			} else if(recvPacket.seqnum==counter){
				counter++;
				ackPacket.ACK=counter;
				printf("Correctly recieved: %s, with seq number :%d \n",recvPacket.data,recvPacket.seqnum);
			} else if(recvPacket.seqnum<counter){
